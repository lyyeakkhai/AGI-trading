...
class BullSpecialist:
    ...
    async def argue(self, symbol: str, timeframe: str, context: str, ta_result: str) -> str:
        if self._client is None and not self.settings.llm.api_key:
            logger.warning("No LLM API key configured. Returning fallback BullThesis.")
            return f"Bullish momentum setup on {symbol}"

        system_prompt = (
            "You are a perma-bull analyst. Your job is to construct the strongest possible "
            "argument for going long, using the provided context and technical analysis."
        )
        user_prompt = (
            f"Argue long for {symbol} on {timeframe} timeframe.\n"
            f"Context: {context}\n"
        ...
        try:
            ...
            if not self.settings.llm.api_key or self.settings.app.env in ("development", "test"):
                logger.warning(
                    f"BullSpecialist LLM call failed or unconfigured ({e}). Falling back."
                )
                return f"Bullish momentum setup on {symbol}"
            raise
