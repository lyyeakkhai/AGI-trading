...
async def test_process_opportunity_valid_proposal(orchestrator):
    ...
    orchestrator.context_assembler.assemble = AsyncMock(return_value={"symbol": "BTC/USDT"})
    ...
    orchestrator.reasoning.evaluate = AsyncMock(return_value=proposal)
    
    # Mock proposal submission
    orchestrator.proposal_client.submit = AsyncMock(return_value={"decision": "Approved"})
