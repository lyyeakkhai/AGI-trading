...
export function PositionsOrdersTable({
  ...
  return (
    <div className="flex flex-col h-full bg-[#181A20] text-xs select-none border-t border-[#23272E]">
      ...
      <div className="h-9 px-3 flex items-center justify-between border-b border-[#23272E] bg-[#181A20] shrink-0">
        {/* Left: Tab list */}
              ...
              <button
                ...
                className={`h-full text-[12px] transition-colors whitespace-nowrap relative border-b-2 flex items-center ${
                  isActive
                    ...
                    : "text-[#848E9C] border-transparent hover:text-[#EAECEF] font-medium"
                }`}
              >
                {tab}
              </button>
        ...
        <div className="flex items-center gap-3 shrink-0 ml-4 font-sans text-[12px] text-[#848E9C]">
          {/* History period selector for history tabs */}
          {activeTab.includes("History") && (
            <div className="flex items-center gap-1 bg-[#1E2329] p-0.5 rounded border border-[#2B313A] text-[10px]">
                ...
                <button
                  ...
                  className={`px-1.5 py-0.5 rounded transition-colors ${
                    historyPeriod === period
                      ? "bg-[#2B313A] text-white font-medium"
            ...
            <div
              className={`w-3.5 h-3.5 rounded-[2px] border transition-colors flex items-center justify-center shrink-0 ${
                hideOtherSymbols
                  ...
                  : "bg-[#1E2329] border-[#474D57] group-hover:border-[#848E9C]"
              }`}
            >
              {hideOtherSymbols && <Check size={10} strokeWidth={3.5} />}
            </div>
            <span className="text-[11px] text-[#848E9C] group-hover:text-[#EAECEF] select-none transition-colors">
              Hide Other Symbols
            </span>
          </button>
      ...
      <div className="flex-1 overflow-x-auto no-scrollbar flex flex-col min-w-0">
        ...
        {activeTab === "Positions(0)" && (
          <div className="min-w-[1645px] grid grid-cols-[140px_100px_105px_115px_105px_100px_100px_100px_120px_120px_105px_135px_80px_130px_80px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
        ...
        {activeTab === "Open Orders(0)" && (
          <div className="min-w-[1300px] grid grid-cols-[140px_110px_90px_70px_110px_110px_100px_110px_140px_90px_90px_90px_80px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
        ...
        {activeTab === "Order History" && (
          <div className="min-w-[1250px] grid grid-cols-[140px_110px_90px_70px_110px_110px_110px_110px_90px_90px_110px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
        ...
        {activeTab === "Trade History" && (
          <div className="min-w-[950px] grid grid-cols-[150px_120px_80px_120px_120px_110px_130px_100px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
        ...
        {activeTab === "Transaction History" && (
          <div className="min-w-[650px] grid grid-cols-[180px_140px_160px_140px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
        ...
        {activeTab === "Position History" && (
          <div className="min-w-[900px] grid grid-cols-[130px_160px_80px_130px_130px_130px_120px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
        ...
        {activeTab === "Bots" && (
          <div className="min-w-[950px] grid grid-cols-[130px_110px_90px_110px_120px_120px_120px_90px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
            {botsColumns.map((col) => (
              <div key={col.key} className={`truncate px-1.5 ${col.align}`}>
                {col.label}
              </div>
        ...
        {activeTab === "Assets" && (
          <div className="min-w-[850px] grid grid-cols-[100px_140px_140px_140px_160px_150px] px-3 py-2 text-[11px] font-sans font-medium text-[#848E9C] border-b border-[#23272E] bg-[#181A20] shrink-0">
        ...
        {activeTab === "Positions(0)" && filteredPositions.length > 0 ? (
          <div className="min-w-[1645px] flex flex-col font-mono text-[12px]">
            {filteredPositions.map((pos) => {
              ...
              return (
                <div
                  key={pos.symbol}
                  className="grid grid-cols-[140px_100px_105px_115px_105px_100px_100px_100px_120px_120px_105px_135px_80px_130px_80px] px-3 py-2 border-b border-[#23272E]/50 hover:bg-[#1E2329]/60 transition-colors items-center text-[#EAECEF]"
                >
                  <div className="flex items-center gap-1 font-semibold text-white px-1.5">
                    <span className="text-[#0ECB81] text-[10px] font-bold">●</span>
                    <span>{pos.symbol}</span>
                    <span className="text-[10px] px-1 bg-[#2B313A] rounded text-[#848E9C] font-normal">
                      Perp
                    </span>
                  </div>
                  <div className={`px-1.5 text-right font-medium ${pos.size > 0 ? "text-[#0ECB81]" : "text-[#F6465D]"}`}>
                    {pos.size > 0 ? `+${pos.size}` : pos.size}
                  </div>
                  <div className="px-1.5 text-right">{pos.entryPrice.toFixed(1)}</div>
                  <div className="px-1.5 text-right text-[#848E9C]">{pos.breakEvenPrice.toFixed(1)}</div>
                  <div className="px-1.5 text-right">{pos.markPrice.toFixed(1)}</div>
                  <div className="px-1.5 text-right text-[#F6465D] font-medium">{pos.liqPrice.toFixed(1)}</div>
                  <div className="px-1.5 text-right text-[#0ECB81]">{pos.marginRatio.toFixed(2)}%</div>
                  <div className="px-1.5 text-right">{pos.margin.toFixed(2)}</div>
                  <div className={`px-1.5 text-right font-semibold ${isProfit ? "text-[#0ECB81]" : "text-[#F6465D]"}`}>
                    {isProfit ? `+${pos.pnl.toFixed(2)}` : pos.pnl.toFixed(2)} ({isProfit ? `+${pos.roi.toFixed(2)}` : pos.roi.toFixed(2)}%)
                  </div>
                  <div className="px-1.5 text-right text-[#848E9C]">{pos.estFundingFee.toFixed(4)}</div>
                  <div className="px-1.5 text-center">
                    <button type="button" className="px-1.5 py-0.5 text-[11px] font-sans bg-[#2B313A] hover:bg-[#363D47] text-[#EAECEF] rounded transition-colors">
                      Market
                    </button>
                  </div>
                  <div className="px-1.5 text-center">
                  ...
                  <div className="px-1.5 text-center">
                    <button type="button" className="p-1 hover:text-[#F0B90B] transition-colors" title="Reverse position">
                      <ArrowUpDown size={12} />
                    </button>
                  </div>
                  <div className="px-1.5 text-center text-[#848E9C] font-sans text-[11px]">
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-8 min-h-[160px] text-[#848E9C]">
...
function EmptyStateGraphic({ title }: { title: string }) {
  return (
    <div className="flex flex-col items-center justify-center select-none">
      ...
      >
        <defs>
          ...
          <linearGradient id="goldFoldGrad" x1="0" y1="0" x2="1" y2="1">
            ...
            <stop offset="100%" stopColor="#FCD535" stopOpacity="0.25" />
          </linearGradient>
        ...
        <line x1="58" y1="36" x2="58" y2="45" stroke="#0ECB81" strokeWidth="0.8" strokeOpacity="0.4" />
        <rect x="56.7" y="38" width="2.6" height="3.5" rx="0.5" fill="#0ECB81" fillOpacity="0.45" />
        ...
        <g transform="translate(64, 49)">
          ...
          <circle cx="15" cy="15" r="14" fill="#F0B90B" fillOpacity="0.04" />
          {/* Magnifying Glass search icon */}
          <circle cx="13.5" cy="13.5" r="5" stroke="#F0B90B" strokeWidth="1.6" />
          <line x1="17.2" y1="17.2" x2="21" y2="21" stroke="#F0B90B" strokeWidth="1.8" strokeLinecap="round" />
        </g>
      ...
      <span className="text-[13px] font-sans font-medium text-[#848E9C] tracking-tight">
        {title}
      </span>
    </div>
  );
