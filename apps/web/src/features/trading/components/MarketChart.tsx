...
export function MarketChart({
  ...
  // Live Chart & Series APIs for SVG overlay sync
  ...
  return (
    <div className={`flex flex-col w-full iron-card border border-[#222B32] border-t-[#384652] rounded-lg shadow-lg overflow-hidden ${className}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 iron-header select-none">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            title="Toggle Hermes AI Drawings & Annotations"
          >
            <Layers size={12} className={showDrawings ? "text-cyan-400" : "text-gray-500"} />
            <span className="hidden sm:inline">DRAWINGS</span>
            {drawings.length > 0 && (
              <span className="ml-0.5 px-1 py-0.2 rounded bg-cyan-500/20 text-[9px] text-cyan-300 font-bold">
                {drawings.length}
              </span>
            )}
          </button>
      ...
      <div
        ref={containerRef}
        className="w-full relative bg-bg-900 overflow-hidden"
        style={{ height }}
      >
        <ChartOverlay
          chart={chartApi}
