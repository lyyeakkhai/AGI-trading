  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 mb-2">
        <span className="text-xs font-semibold uppercase tracking-wider text-gray-300 truncate">
          {label}
        </span>
      ...
      <div className="flex items-baseline gap-2">
        <div className={`font-mono tabular-nums font-bold tracking-tight text-gray-100 ${valueSizes[size]}`}>
          {value}
        </div>
      </div>
      ...
      {(change !== undefined || subtext) && (
        <div className="flex items-center gap-2 mt-2 pt-2 border-t border-border-color/60 text-xs">
          {change !== undefined && (
            <span className={`inline-flex items-center font-mono tabular-nums font-medium gap-0.5 ${trendClass}`}>
