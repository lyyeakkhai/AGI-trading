     ...
     return (
      <React.Fragment key={step.id}>
       <div className="flex flex-col items-center gap-1.5 min-w-[70px] text-center">
        <div
         className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-sans font-bold transition-all ${
          isCompleted
           ? "bg-profit/20 border border-profit text-profit"
           : isActive
           ? "bg-cyan-500/20 border border-cyan-400 text-indigo-600 dark:text-indigo-400 shadow-[0_0_8px_rgba(0,229,255,0.4)] animate-pulse"
        ...
        >
         {isCompleted ? (
          ...
          <Circle size={8} className="fill-cyan-400 text-indigo-600 dark:text-indigo-400" />
         ) : (
          <span>{index + 1}</span>
         )}
        </div>
